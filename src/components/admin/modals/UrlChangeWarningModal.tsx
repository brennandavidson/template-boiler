...
interface UrlChangeWarningModalProps {
  ...
  oldUrl: string;
  newUrl: string;
  changeType: 'slug' | 'category' | 'both';
  isCircularRedirect?: boolean;
}

export default function UrlChangeWarningModal({
  isOpen,
  onClose,
  onConfirm,
  oldUrl,
  ...
  const getChangeDescription = () => {
    switch (changeType) {
      case 'slug':
        return "You're changing the post slug";
      case 'category':
        return "You're changing the post category";
      case 'both':
        return "You're changing both the slug and category";
      default:
        return "You're changing the URL";
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-xl shadow-2xl max-w-lg w-full overflow-hidden">
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="bg-gray-50 rounded-lg p-4 space-y-3">
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium text-gray-500 mb-1">Current URL</p>
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium text-gray-500 mb-1">New URL</p>
          ...
          <div className={`border rounded-lg p-4 ${ isCircularRedirect ?'bg-green-50 border-green-200':'bg-primary-50 border-primary-200'}`}>
            <div className="flex gap-3">
              <svg className={`w-5 h-5 flex-shrink-0 mt-0.5 ${ isCircularRedirect ?'text-green-600':'text-primary-600'}`} fill="currentColor" viewBox="0 0 20 20">
              ...
              <div className="flex-1">
                <p className={`text-sm font-medium mb-1 ${ isCircularRedirect ?'text-green-900':'text-primary-900'}`}>
                  {isCircularRedirect ? 'Smart Redirect Management' : 'SEO Impact'}
                </p>
                <p className={`text-xs leading-relaxed ${ isCircularRedirect ?'text-green-800':'text-primary-800'}`}>
                  {isCircularRedirect 
        ...
        <div className="bg-gray-50 px-6 py-4 flex gap-3">
          <button
            ...
            className="flex-1 px-4 py-2.5 bg-white text-gray-700 font-medium rounded-lg border border-gray-300 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
          ...
          <button
            ...
            disabled={isCreatingRedirect}
            className="flex-1 px-4 py-2.5 bg-primary-600 text-white font-medium rounded-lg hover-700 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isCreatingRedirect ? (
